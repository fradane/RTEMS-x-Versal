...
\hline
Berta Alice      & XXXXXXXX & \texttt{alice.berta@mail.polimi.it} \\
...
\item
  \textbf{Source code:} [\todo[inline]{Insert repository URL when available}
  REPO URL (not yet publicly available)]
...
\begin{itemize}
  ...
  \item \textbf{Memory protection and low-level debugging.}
        ...
        to add the missing region descriptor, all skills squarely within
        the scope of OS-level systems programming.
...
[\todo[inline]{Insert repository URL}].
...
\begin{itemize}
  ...
  \item \textbf{BSP patch: MPU region for GPIO peripheral.}
        ...
        This fix has been documented and may be useful for upstream submission.
        [\todo[inline]{Add link to patch / commit if submitted upstream}]
  ...
  \item \textbf{Documentation.}
        A full technical report (separate from this template document) describes
        ...
        material covering this toolchain setup from scratch.
        Whether this makes sense as an upstream contribution and how to proceed
        is an open question to be assessed with members of the RTEMS community.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Open questions.}
        One question that remained unanswered was whether the BSP MPU patch is
        the canonical approach expected by the RTEMS community or whether there
        is a higher-level API (e.g.\ a BSP configuration option) to declare
        additional peripheral regions without modifying BSP source directly.
\end{itemize}
...
This confirmed that RTEMS was actually running; the problem was isolated to
the console driver.
